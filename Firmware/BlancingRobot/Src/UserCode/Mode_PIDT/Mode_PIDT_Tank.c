...
static void load_pid_params(){
	...
	mavlink_msg_pid_params_pack(0,0,&msg,PID_WHE0,params.pid_whe0.KP,params.pid_whe0.KI,params.pid_whe0.KD);
	len = mavlink_msg_to_send_buffer(gmav_send_buf, &msg);
	com_send(gmav_send_buf, len);
	pid_reset(&params.pid_whe0);

	mavlink_msg_pid_params_pack(0,0,&msg,PID_WHE1,params.pid_whe1.KP,params.pid_whe1.KI,params.pid_whe1.KD);
	len = mavlink_msg_to_send_buffer(gmav_send_buf, &msg);
	com_send(gmav_send_buf, len);
...
static void write_param(mavlink_message_t *msg){
	...
	case PID_WHE0:
		params.pid_whe0.KP = pid_msg.KP;
		params.pid_whe0.KI = pid_msg.KI;
		params.pid_whe0.KD = pid_msg.KD;
		pid_reset(&params.pid_whe0);
		break;
	case PID_WHE1:
		params.pid_whe1.KP = pid_msg.KP;
		params.pid_whe1.KI = pid_msg.KI;
		params.pid_whe1.KD = pid_msg.KD;
		pid_reset(&params.pid_whe1);
		break;
	case PID_SYNC:
...
static void pid_report_callback(uint8_t *ctx){
	...
	mavlink_msg_pid_report_pack(0,0,&pid_report_msg,PID_WHE0,
			params.pid_whe0.sp,
			params.pid_whe0.fb,
			params.pid_whe0.P_Part,
			params.pid_whe0.I_Part,
			params.pid_whe0.D_Part,
			params.pid_whe0.U);
	...
	mavlink_msg_pid_report_pack(0,0,&pid_report_msg,PID_WHE1,
			params.pid_whe1.sp,
			params.pid_whe1.fb,
			params.pid_whe1.P_Part,
			params.pid_whe1.I_Part,
			params.pid_whe1.D_Part,
			params.pid_whe1.U);
	...
	mavlink_msg_pid_report_pack(0,0,&pid_report_msg,PID_SYNC,
			params.pid_sync.sp,
			params.pid_sync.fb,
			params.pid_sync.P_Part,
			params.pid_sync.I_Part,
			params.pid_sync.D_Part,
			params.pid_sync.U);
	len = mavlink_msg_to_send_buffer(gmav_send_buf, &pid_report_msg);
	com_send(gmav_send_buf, len);
}
