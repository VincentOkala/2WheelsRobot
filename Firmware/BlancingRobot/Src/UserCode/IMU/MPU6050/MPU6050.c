...
static void mpu6050_read(uint8_t DevAddress, uint8_t MemAddress, uint8_t* value, uint8_t mask){
	HAL_I2C_Mem_Read(&MPU6050_I2C, DevAddress, MemAddress, 1, value, 1, 20);
	*value &= mask;
}

static void mpu6050_write(uint8_t DevAddress, uint8_t MemAddress, uint8_t value, uint8_t mask){
	value &= mask;
	HAL_I2C_Mem_Write(&MPU6050_I2C, DevAddress, MemAddress, 1, &value, 1, 20);
}

static void mpu6050_read_bytes(uint8_t DevAddress, uint8_t MemAddress, uint8_t* value, uint8_t len){
	HAL_I2C_Mem_Read(&MPU6050_I2C, DevAddress, MemAddress, 1, value, len, 20);
}
 ...
 */
void MPU6050_init() {
	mpu6050_set_clock_source(MPU6050_PLL_X_GYRO);
 ...
 */
void mpu6050_set_full_scale_gyro_range(mpu6050_fs_sel_t fs_sel) {
	mpu6050_write(MPU6050_ADDRESS, MPU6050_RA_GYRO_CONFIG, fs_sel, MPU6050_GYRO_CONFIG_FS_SEL);
}
 ...
 */
void mpu6050_set_full_scale_accel_range(mpu6050_afs_sel_t afs_sel) {
    mpu6050_write(MPU6050_ADDRESS, MPU6050_RA_ACCEL_CONFIG, afs_sel, MPU6050_ACCEL_CONFIG_AFS_SEL);
}
...
void mpu6050_get_motion(int16_t* ax, int16_t* ay, int16_t* az, int16_t* gx, int16_t* gy, int16_t* gz) {
	uint8_t buffer[14] = {};
	mpu6050_read_bytes(MPU6050_ADDRESS, MPU6050_RA_ACCEL_XOUT_H, buffer, 14);
 ...
 */
void mpu6050_set_sleep_enabled(bool enabled) {
	mpu6050_write(MPU6050_ADDRESS, MPU6050_RA_PWR_MGMT_1, enabled << 3, MPU6050_PWR1_SLEEP);
}
 ...
 */
void mpu6050_set_clock_source(mpu6050_clock_source_t clock_source){
	mpu6050_write(MPU6050_ADDRESS, MPU6050_RA_PWR_MGMT_1, clock_source, MPU6050_PWR1_CLKSEL);
}
...
uint8_t mpu6050_get_device_ID(){
	uint8_t who_am_i = 0;
	mpu6050_read(MPU6050_ADDRESS, MPU6050_RA_WHO_AM_I, &who_am_i, MPU6050_WHO_AM_I);
    return who_am_i >> 1;
}
...
//	readBytes(devAddr, SaveAddress, 2, buffer);
//	return (((int16_t)buffer[0]) << 8) | buffer[1];
//}
